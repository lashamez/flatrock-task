...
RUN mkdir -p /app/order/.m2/repository/

# Copy the built jar file from the flatrock_common image
COPY --from=flatrock_common /app/common/target/common-1.0-SNAPSHOT.jar /app/order/.m2/repository/com/flatrock/common/1.0-SNAPSHOT/common-1.0-SNAPSHOT.jar

# Set the environment variable for the classpath
ENV CLASSPATH /app/order/.m2/repository/
...
RUN cd /app/order && ./mvnw clean package -DskipTests -Dmaven.repo.local=$CLASSPATH
...
COPY --from=builder /app/order/target/order-0.0.1-SNAPSHOT.jar /app/order-0.0.1-SNAPSHOT.jar
